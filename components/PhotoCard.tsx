...
interface PhotoCardProps {
  photo: SelectedPhoto;
  onSupercharge?: (photoId: string) => void; // Optional callback for the new feature
  isSupercharging?: boolean; // To show loading state
}

const PhotoCard: React.FC<PhotoCardProps> = ({ photo, onSupercharge, isSupercharging }) => {
  // Use the enhanced URL if it exists, otherwise fall back to the original object URL
  const displayUrl = photo.enhancedObjectURL || photo.objectURL;
    ...
    <div className="bg-slate-700 bg-opacity-70 rounded-xl shadow-xl overflow-hidden flex flex-col h-full transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        {onSupercharge && (
          <div className="mt-4">
            <Button
              onClick={() => onSupercharge(photo.id)}
              disabled={isSupercharging}
              variant="primary"
              size="small"
              className="w-full"
            >
              {isSupercharging ? 'Supercharging...' : 'Supercharge My Looks'}
            </Button>
          </div>
        )}
      </div>
